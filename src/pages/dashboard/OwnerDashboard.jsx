...
const OwnerDashboard = () => {
  ...
  // Debug : afficher l'utilisateur courant dans la console
  console.log('currentUser', currentUser);
  console.log('OwnerDashboard rendu', { loading, currentUser, boats, reservations });

  if (loading || !currentUser) {
    return (
      <div style={{ minHeight: '100vh', background: '#274991', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
        <div style={{ fontSize: 32, marginBottom: 24 }}>Chargement du tableau de bord propriétaire…</div>
        {!loading && !currentUser && (
          <>
            <div style={{ fontSize: 20, marginBottom: 16 }}>Erreur d'authentification. Veuillez vous reconnecter.</div>
            <button onClick={logout} style={{ padding: '12px 32px', fontSize: 18, background: 'white', color: '#274991', border: 'none', borderRadius: 8, cursor: 'pointer', fontWeight: 600 }}>Se reconnecter</button>
          </>
        )}
      </div>
    );
  ...
  return (
    <div>
      <div style={{padding: 20}}>TEST CONTENU DASHBOARD</div>
      <div className="min-h-screen bg-background">
        ...
        <header className="bg-white shadow-md">
          <div className="container mx-auto px-4 py-4 flex justify-between items-center">
            ...
            <div className="flex items-center">
              <span className="text-dark mr-4">Bonjour, {currentUser?.firstName || currentUser?.name || 'Propriétaire'}</span>
              <button 
                onClick={handleLogout}
                className="bg-gray-400 hover:bg-gray-500 text-dark py-2 px-4 rounded-md transition-colors"
              >
                Déconnexion
              </button>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="card p-6 md:col-span-1">
              ...
              <div className="flex flex-col space-y-4">
                <div>
                  ...
                  <p className="font-medium">{(currentUser?.firstName || '') + (currentUser?.lastName ? ' ' + currentUser.lastName : '') || currentUser?.name || 'Non défini'}</p>
                </div>
                <div>
            ...
            <div className="card p-6 md:col-span-2">
              <div className="flex justify-between items-center mb-4">
                ...
                <button 
                  onClick={() => navigate('/owner/dashboard/add-boat')} 
